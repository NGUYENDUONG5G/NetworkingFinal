...
class LSrouter(Router):
    ...
    def handle_packet(self, port, packet):
        ...
        if packet.is_traceroute:
            ...
            # Routing packet: content = (origin, seq, links)
            origin, seq, links = packet.content
            # If this LSP is newer, update and flood
            ...
            if prev is None or seq > prev[0]:
                ...
                # Recompute routes
                self.run_dijkstra()
                # Flood to all neighbors except where received
                for p, (nbr, _) in self.neighbors.items():
                    if p != port:
                        pkt = Packet(Packet.ROUTING, self.addr, nbr,
                                     content=(origin, seq, links.copy()))
                        self.send(p, pkt)

    def handle_new_link(self, port, endpoint, cost):
        """Add a new link and advertise updated LSP."""
        self.neighbors[port] = (endpoint, cost)
        self.seq_num += 1
        self.advertise_lsp()

    def handle_remove_link(self, port):
        """Remove a link and advertise updated LSP."""
    ...
    def handle_time(self, time_ms):
        """Periodic heartbeat to refresh LSP."""
    ...
    def advertise_lsp(self):
        """Create and flood this router's LSP to all neighbors."""
        # Build link-state info
        ...
        for port, (nbr, _) in self.neighbors.items():
            pkt = Packet(Packet.ROUTING, self.addr, nbr,
                         content=(self.addr, self.seq_num, links.copy()))
            self.send(port, pkt)
        ...
        # Build forwarding table: dest -> (port, cost)
